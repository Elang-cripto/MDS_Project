...
function toggleNavLinksColor(type) {
    ...
    if (type === "blur") {
        ...
        navLinks.forEach((element) => {
            element.classList.add("text-default");
        });
...
function navbarBlurOnScroll(id) {
    ...
    let classes = ["bg-default", "left-auto", "position-sticky"];
...
function darkMode(el) {
    ...
    const sidebarWhite = document.querySelectorAll(".sidenav.bg-default");
    ...
    if (!el.getAttribute("checked")) {
        ...
        for (var i = 0; i < sidebarWhite.length; i++) {
            sidebarWhite[i].classList.remove("bg-default");
        }
